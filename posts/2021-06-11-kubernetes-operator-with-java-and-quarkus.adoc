...
* Technologies such as containers, microservices, immutable infrastructure, deployed via declarative code are common elements cloud-native computing approach. It is a good fit for writing an operator which needs to be deployed in a container, and the state of resources managed by an operator is defined in a declarative way. Quarkus with the main focus on cloud-native, and microservice solutions looks like a good fit for building an operator.
